...
Build production-quality user interfaces that are accessible, performant, and visually polished. The goal is UI that looks like it was built by a design-aware engineer at a top company ΓÇö not like it was generated by an AI. This means real design system adherence, proper accessibility, thoughtful interaction patterns, and no generic "AI aesthetic."
...
```
Local state (useState)           ΓåÆ Component-specific UI state
Lifted state                     ΓåÆ Shared between 2-3 sibling components
Context                          ΓåÆ Theme, auth, locale (read-heavy, write-rare)
URL state (searchParams)         ΓåÆ Filters, pagination, shareable UI state
Server state (React Query, SWR)  ΓåÆ Remote data with caching
Global store (Zustand, Redux)    ΓåÆ Complex client state shared app-wide
```
...
```
h1 ΓåÆ Page title (one per page)
h2 ΓåÆ Section title
h3 ΓåÆ Subsection title
body ΓåÆ Default text
small ΓåÆ Secondary/helper text
```
...
- Use semantic color tokens: `text-primary`, `bg-surface`, `border-default` ΓÇö not raw hex values
...
<button onClick={handleClick}>Click me</button>        // Γ£ô Focusable by default
<div onClick={handleClick}>Click me</div>               // Γ£ù Not focusable
<div role="button" tabIndex={0} onClick={handleClick}    // Γ£ô But prefer <button>
     onKeyDown={e => {
