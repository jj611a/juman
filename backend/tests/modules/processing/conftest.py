"""Fixtures for Processing module tests."""
...
@pytest.fixture
async def laundry_inspection_item(
    ...
    """Completed inspection item requiring laundry; dress in PROCESSING."""
    rental = await rental_service.create(
        customer_id=sample_customer.id,
        rental_at=utc(2026, 8, 1, 10),
        expected_return_at=utc(2026, 8, 10, 10),
    ...
    return [i for i in insp.items if not i.is_deleted][0]
...
async def admin_client(
    ...
    _, token = await mint_admin_bearer(db_session, username="proc_admin")
