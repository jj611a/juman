...
import type { SizeDto } from '@/services/domainTypes'
import { useSizesList, useCreateSize, useDeleteSize, useUpdateSize } from '../hooks'
...
export default function SizesListPage(): React.ReactElement {
  ...
  const [selected, setSelected] = React.useState<SizeDto | null>(null)
  const [formDirty, setFormDirty] = React.useState(false)
  const [discardOpen, setDiscardOpen] = React.useState(false)
  const [deleteTarget, setDeleteTarget] = React.useState<SizeDto | null>(null)
  ...
  const listQuery = useSizesList(listParams)
  const createMutation = useCreateSize()
  const updateMutation = useUpdateSize(selected?.id ?? '')
  const deleteMutation = useDeleteSize()

  const columns = React.useMemo(
    () => [
      createDataColumn<SizeDto>({
        accessorKey: 'name_ar',
        header: 'الاسم'
      }),
      createDataColumn<SizeDto>({
  ...
  const openEdit = (row: SizeDto): void => {
    setSelected(row)
    setFormDirty(false)
    setDrawerMode('edit')
  }
  ...
  const actions = React.useMemo<DataRowAction<SizeDto>[]>(
    () => [
      {
        id: 'edit',
        label: 'تعديل',
