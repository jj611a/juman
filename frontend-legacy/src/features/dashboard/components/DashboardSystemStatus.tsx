...
export function DashboardSystemStatus(): React.ReactElement | null {
  ...
  const loading =
    (canSystem && (health.isLoading || version.isLoading)) || (canBackup && backups.isLoading)
  const healthError = canSystem && health.isError
  const latestBackup = backups.data?.data?.[0]
  const backupsUnsupported = backups.isError && isV2Unsupported(backups.error)
        ...
        {canBackup ? (
          <div className="flex justify-between gap-2">
            ...
            <dd className="text-base-content">
              {backupsUnsupported ? (
                <span className="text-base-content/50">غير متاح في V2</span>
              ) : backups.isError ? (
