...
export default function CalendarHomePage(): React.ReactElement {
  ...
  return (
    <Page size="lg" as="main">
      <PageHeader
        title="التقويم"
        description="واجهة كتل التقويم غير مربوطة بـ Nest V2 بعد."
      />
      <InlineMessage variant="info" className="mb-4">
        التوفر يُتحقق على الخادم عند تأكيد الحجز أو إنشاء التأجير. لا يوجد مسار HTTP لعرض الجدول
        حالياً.
      </InlineMessage>
          ...
          {(listQuery.data?.data ?? []).map((d) => (
            <li key={d.id} className="flex items-center justify-between gap-4 px-4 py-3">
              ...
              <Button
                type="button"
                variant="secondary"
                onClick={() => void navigate(`/inventory/${d.id}`)}
              >
                فتح المخزون
              </Button>
            </li>
