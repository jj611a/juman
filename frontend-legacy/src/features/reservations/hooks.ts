...
import { reservationKeys, reservationsApi, type ReservationCheckoutBody } from './api'
...
export function useCheckoutReservation() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, body }: { id: string; body?: ReservationCheckoutBody }) => {
      if (!guardOnline()) return Promise.reject({ code: 'OFFLINE', message: 'غير متصل' })
      return reservationsApi.checkout(id, body)
    },
    onSuccess: (_d, vars) => {
      toast.success('تم تحويل الحجز لتأجير')
      invalidate(qc, vars.id)
    },
    onError: (e) => toastAppError(e, 'فشل تحويل الحجز')
  })
}
