...
import {
  ...
  mapRentalBodyToV2,
  mapRentalUpdateBodyToV2,
  mapRentalV2ToLegacy,
  mapAuditLogV2ToLegacy,
...
export const apiClient = {
  ...
  categories: {
    list: async (params?: CategoryListParams): Promise<ListEnvelope<CategoryDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: '/categories',
        query: bridgeTaxonomyListQuery(params as Record<string, unknown>)
      })
      const page = unwrapV2Page<V2Category>(raw)
      let items = page.items.map(mapCategoryV2ToLegacy)
      // Nest taxonomy list has no status/sort whitelist; filter active client-side.
      if (params?.active_only === true) {
        items = items.filter((c) => c.is_active)
        return toLegacyList(items, { ...page.meta, total: items.length }, (x) => x)
      }
  ...
  brands: {
    list: async (params?: BrandListParams): Promise<ListEnvelope<BrandDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: '/brands',
        query: bridgeTaxonomyListQuery(params as Record<string, unknown>)
      })
      const page = unwrapV2Page<V2Brand>(raw)
      let items = page.items.map(mapBrandV2ToLegacy)
      if (params?.active_only === true) {
        items = items.filter((c) => c.is_active)
        return toLegacyList(items, { ...page.meta, total: items.length }, (x) => x)
      }
      return toLegacyList(page.items, page.meta, mapBrandV2ToLegacy)
    },
    get: async (id: string): Promise<ItemEnvelope<BrandDto>> => {
      const raw = await invoke<V2Brand>({ method: 'GET', path: `/brands/${id}` })
      return toLegacyItem(raw, mapBrandV2ToLegacy)
    },
    create: async (body: BrandCreateBody): Promise<ItemEnvelope<BrandDto>> => {
      const raw = await invoke<V2Brand>({
        method: 'POST',
        path: '/brands',
        body: mapCategoryBodyToV2(body as unknown as Record<string, unknown>)
      })
    ...
    update: async (id: string, body: BrandUpdateBody): Promise<ItemEnvelope<BrandDto>> => {
      const raw = await invoke<V2Brand>({
        method: 'PATCH',
        path: `/brands/${id}`,
        body: mapCategoryBodyToV2(body as unknown as Record<string, unknown>)
    ...
    remove: async (id: string): Promise<MessageEnvelope> => {
      await invoke({ method: 'DELETE', path: `/brands/${id}` })
      return toLegacyMessage()
    }
  },

  colors: {
    list: async (params?: ColorListParams): Promise<ListEnvelope<ColorDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: '/colors',
        query: bridgeTaxonomyListQuery(params as Record<string, unknown>)
      })
      const page = unwrapV2Page<V2Color>(raw)
      let items = page.items.map(mapColorV2ToLegacy)
      if (params?.active_only === true) {
        items = items.filter((c) => c.is_active)
        return toLegacyList(items, { ...page.meta, total: items.length }, (x) => x)
      }
      return toLegacyList(page.items, page.meta, mapColorV2ToLegacy)
    },
    get: async (id: string): Promise<ItemEnvelope<ColorDto>> => {
      const raw = await invoke<V2Color>({ method: 'GET', path: `/colors/${id}` })
      return toLegacyItem(raw, mapColorV2ToLegacy)
    },
    create: async (body: ColorCreateBody): Promise<ItemEnvelope<ColorDto>> => {
      const raw = await invoke<V2Color>({
        method: 'POST',
        path: '/colors',
        body: mapColorBodyToV2(body as unknown as Record<string, unknown>)
      })
      return toLegacyItem(raw, mapColorV2ToLegacy)
    },
    update: async (id: string, body: ColorUpdateBody): Promise<ItemEnvelope<ColorDto>> => {
      const raw = await invoke<V2Color>({
        method: 'PATCH',
        path: `/colors/${id}`,
        body: mapColorBodyToV2(body as unknown as Record<string, unknown>)
    ...
    remove: async (id: string): Promise<MessageEnvelope> => {
      await invoke({ method: 'DELETE', path: `/colors/${id}` })
      return toLegacyMessage()
    }
  },

  sizes: {
    list: async (params?: SizeListParams): Promise<ListEnvelope<SizeDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: '/sizes',
        query: bridgeTaxonomyListQuery(params as Record<string, unknown>)
      })
      const page = unwrapV2Page<V2Size>(raw)
      let items = page.items.map(mapSizeV2ToLegacy)
      if (params?.active_only === true) {
        items = items.filter((c) => c.is_active)
        return toLegacyList(items, { ...page.meta, total: items.length }, (x) => x)
      }
      return toLegacyList(page.items, page.meta, mapSizeV2ToLegacy)
    },
    get: async (id: string): Promise<ItemEnvelope<SizeDto>> => {
      const raw = await invoke<V2Size>({ method: 'GET', path: `/sizes/${id}` })
      return toLegacyItem(raw, mapSizeV2ToLegacy)
    },
    create: async (body: SizeCreateBody): Promise<ItemEnvelope<SizeDto>> => {
      const raw = await invoke<V2Size>({
        method: 'POST',
        path: '/sizes',
        body: mapCategoryBodyToV2(body as unknown as Record<string, unknown>)
      })
    ...
    update: async (id: string, body: SizeUpdateBody): Promise<ItemEnvelope<SizeDto>> => {
      const raw = await invoke<V2Size>({
        method: 'PATCH',
        path: `/sizes/${id}`,
        body: mapCategoryBodyToV2(body as unknown as Record<string, unknown>)
    ...
    remove: async (id: string): Promise<MessageEnvelope> => {
      await invoke({ method: 'DELETE', path: `/sizes/${id}` })
      return toLegacyMessage()
    }
  },

  finance: {
    listAccounts: async (params?: FinanceListParams): Promise<ListEnvelope<FinanceAccountDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: '/finance/accounts',
        query: bridgeListQuery(params as Record<string, unknown>)
      })
      const page = unwrapV2Page<FinanceAccountDto>(raw)
      return toLegacyList(page.items, page.meta, (x) => x)
    },
    listTransactions: async (
      params?: FinanceListParams
    ): Promise<ListEnvelope<FinanceTransactionDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: '/finance/transactions',
        query: bridgeListQuery(params as Record<string, unknown>)
      })
      const page = unwrapV2Page<FinanceTransactionDto>(raw)
      return toLegacyList(page.items, page.meta, (x) => x)
    },
    listPayments: async (params?: FinanceListParams): Promise<ListEnvelope<FinancePaymentDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: '/finance/payments',
        query: bridgeListQuery(params as Record<string, unknown>)
      })
      const page = unwrapV2Page<FinancePaymentDto>(raw)
      return toLegacyList(page.items, page.meta, (x) => x)
    },
    getOutstanding: async (params: {
      accountId?: string
      customerId?: string
    }): Promise<FinanceOutstandingDto> => {
      return invoke<FinanceOutstandingDto>({
        method: 'GET',
  ...
  barcodes: {
    list: async (params?: BarcodeListParams): Promise<ListEnvelope<BarcodeDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: '/barcodes',
        query: bridgeListQuery(params as Record<string, unknown>)
      })
      const page = unwrapV2Page<BarcodeDto>(raw)
      return toLegacyList(page.items, page.meta, (x) => x)
    },
    get: async (id: string): Promise<ItemEnvelope<BarcodeDto>> => {
      const raw = await invoke<BarcodeDto>({ method: 'GET', path: `/barcodes/${id}` })
      return toLegacyItem(raw, (x) => x)
    },
    generate: async (body?: BarcodeGenerateBody): Promise<ItemEnvelope<BarcodeDto>> => {
      const raw = await invoke<BarcodeDto>({
        method: 'POST',
    ...
    validate: async (body: BarcodeValidateBody): Promise<BarcodeValidateResult> => {
      return invoke<BarcodeValidateResult>({
        method: 'POST',
        path: '/barcodes/validate',
        body
      })
    },
    reserve: async (body?: BarcodeReserveBody): Promise<ItemEnvelope<BarcodeDto>> => {
      const raw = await invoke<BarcodeDto>({
        method: 'POST',
        path: '/barcodes/reserve',
        body: body ?? {}
      })
      return toLegacyItem(raw, (x) => x)
    },
    release: async (body: BarcodeValueBody): Promise<ItemEnvelope<BarcodeDto>> => {
      const raw = await invoke<BarcodeDto>({
        method: 'POST',
        path: '/barcodes/release',
        body
      })
      return toLegacyItem(raw, (x) => x)
    },
    retire: async (body: BarcodeValueBody): Promise<ItemEnvelope<BarcodeDto>> => {
      const raw = await invoke<BarcodeDto>({
        method: 'POST',
        path: '/barcodes/retire',
        body
      })
      return toLegacyItem(raw, (x) => x)
    }
  ...
  dresses: {
    ...
    updateBarcode: async (
      ...
      const raw = await invoke<V2Item>({
        method: 'PATCH',
        path: `/items/${id}`,
        body: code ? { barcode: code } : { generateBarcode: true }
      })
      return toLegacyItem(raw, mapItemV2ToDress)
    }
  },
  ...
  dressPhotos: {
    list: async (dressId: string): Promise<{ success: boolean; data: DressPhotoDto[] }> => {
      const raw = await invoke<V2Item>({ method: 'GET', path: `/items/${dressId}` })
      const media = raw.media ?? []
      const data: DressPhotoDto[] = media.map((m) => ({
        id: m.id,
        dress_id: dressId,
        stored_file_id: m.mediaFileId,
        display_order: m.displayOrder ?? 0,
        is_cover: Boolean(m.isPrimary),
        caption: m.purpose ?? null,
        file: m.mediaFile
          ? {
              id: m.mediaFile.id,
              original_filename: m.mediaFile.originalFilename,
              mime_type: m.mediaFile.mimeType,
              size_bytes: 0
            }
          : null,
        created_at: '',
    ...
    create: async (
      dressId: string,
      body: DressPhotoCreateBody
    ): Promise<ItemEnvelope<DressPhotoDto>> => {
      await invoke({
        method: 'POST',
        path: `/items/${dressId}/media`,
        body: {
          mediaFileId: body.stored_file_id,
          isPrimary: body.is_cover ?? false,
          purpose: body.caption ?? undefined,
          displayOrder: body.display_order ?? undefined
        }
      })
      const raw = await invoke<V2Item>({ method: 'GET', path: `/items/${dressId}` })
      const m =
        (raw.media ?? []).find((x) => x.mediaFileId === body.stored_file_id) ??
        (raw.media ?? [])[0]
      if (!m) {
        throw { code: 'NOT_FOUND', message: 'Media attach succeeded but photo not returned' }
      }
      return {
        success: true,
        data: {
    ...
    setCover: async (dressId: string, photoId: string): Promise<ItemEnvelope<DressPhotoDto>> => {
      const raw = await invoke<V2Item>({ method: 'GET', path: `/items/${dressId}` })
      const m = (raw.media ?? []).find((x) => x.id === photoId)
      if (!m) throw { code: 'NOT_FOUND', message: 'Photo not found' }
      await invoke({
        method: 'POST',
        path: `/items/${dressId}/media`,
        body: { mediaFileId: m.mediaFileId, isPrimary: true }
      })
      return {
        success: true,
        data: {
    ...
    reorder: (
      ...
      v2Unsupported('dressPhotos.reorder — Nest has no reorder HTTP'),
    remove: (_photoId: string): Promise<MessageEnvelope> =>
      v2Unsupported('dressPhotos.remove — Nest has no item-media detach HTTP')
  },
  ...
  rentals: {
    ...
    update: async (id: string, body: RentalUpdateBody): Promise<ItemEnvelope<RentalDto>> => {
      const raw = await invoke<V2Rental>({
        method: 'POST',
        path: `/rentals/${id}/notes`,
        body: mapRentalUpdateBodyToV2(body)
      })
      if (!raw || typeof raw !== 'object' || !('id' in raw)) {
        throw { code: 'INTERNAL', message: 'استجابة تحديث الملاحظات غير صالحة' }
      }
      return toLegacyItem(raw, mapRentalV2ToLegacy)
    },
    audit: async (id: string): Promise<ListEnvelope<AuditLogDto>> => {
      const raw = await invoke({
        method: 'GET',
        path: `/rentals/${id}/audit`,
        query: { limit: 50, offset: 0 }
      })
      const page = unwrapV2Page<Record<string, unknown>>(raw)
      return toLegacyList(page.items, page.meta, mapAuditLogV2ToLegacy)
    },
    checkout: async (
      id: string,
      body?: { depositAmountFils?: number; reason?: string; idempotencyKey?: string }
