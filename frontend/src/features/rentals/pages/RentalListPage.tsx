...
import type { RentalDto } from '../api/api'
import { 
  Plus, 
  Search, 
  Eye, 
...
export function RentalListPage() {
  ...
  // List Query Hook
  const { data, isLoading, isError, refetch } = useRentalsList({
    q: q.trim() || undefined,
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-base-content/5 pb-4">
        <div>
          <h1 className="text-2xl font-black text-base-content flex items-center gap-2">
            ...
            عقود تأجير الفساتين والقطع
          </h1>
          <p className="text-xs text-base-content/50 mt-1">
            إدارة عقود إيجار الفساتين الحالية للعملاء ومتابعة تواريخ تسليمها وإرجاعها للمخزن
          </p>
        </div>
        <div className="flex gap-2 shrink-0">
          {can(PERMISSION.RENTAL_CREATE) && (
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-4 bg-base-300/40 p-4 rounded-xl border border-base-content/5 items-end">
        <div className="form-control w-full col-span-2">
          ...
          <div className="relative">
            <input
              type="text"
              placeholder="ابحث برقم عقد التأجير..."
        ...
        <div className="form-control w-full">
          <span className="label-text mb-1.5 text-xs text-base-content/50 font-semibold">حالة العقد</span>
          <select
          ...
          >
            ...
            <option value="return_pending">بانتظار الإرجاع (Return Pending)</option>
            ...
            <tbody>
              {(data?.data || []).map((rent) => (
                <tr key={rent.id} className="border-b border-base-content/5 hover:bg-base-200/40 transition-colors">
                  ...
                  <td>
                    <span className="badge badge-neutral badge-xs font-mono text-[9px] py-2">{rent.status}</span>
                  </td>
                  <td className="text-left flex justify-end gap-1.5">
                    <button
                      onClick={() => navigate(`/rentals/${rent.id}`)}
