...
describe('MoneyDisplay', () => {
  it('formats positive, negative, and zero', () => {
    ...
    expect(screen.getByText(/1\.000 IQD/).getAttribute('data-sign')).toBe('positive')

    rerender(<MoneyDisplay value={-1_000} />)
    expect(screen.getByText(/-1\.000 IQD/).getAttribute('data-sign')).toBe('negative')

    rerender(<MoneyDisplay value={0} />)
    expect(screen.getByText(/0\.000 IQD/).getAttribute('data-sign')).toBe('zero')
  })

  it('supports compact mode', () => {
    render(<MoneyDisplay value={1_000_000} compact />)
    expect(screen.getByText('1000 IQD')).toBeInTheDocument()
  })
})
